...
@Table(name = "message_emoji",
    uniqueConstraints = @UniqueConstraint(
        name = "uk_message_emoji",
        columnNames = {"user_id", "message_id", "emoji_type"} // 중복 없이 여러 이모지 가능.
    ))
public class MessageEmoji {
    ...
    public static MessageEmoji of(User user, Messages message, EmojiType type) {
        MessageEmoji e = new MessageEmoji();
        e.user = user;
        e.message = message;
        e.emojiType = type;
        return e;
