...
| 좋아요/댓글 | `post_likes` / `post_comments` 별도 테이블 | 댓글은 `parent_id`로 대댓글(1단계) 지원. 좋아요는 #182에서 복구 |
...
-- ──────────────────────────────────────────────────────────────
-- post_likes (좋아요)
--   #145~#148에서 댓글 이모지로 반응 모델을 단일화하며 한때 미채택 처리했으나,
--   #182에서 다시 복구했다. 자바 엔티티/리포지토리/서비스와 컨트롤러가 이 테이블을 사용한다.
-- ──────────────────────────────────────────────────────────────
