...
## 5. 🟠 Swagger가 운영에서도 무조건 열린다

`SecurityConfig`가 `/swagger-ui/**` · `/v3/api-docs/**`를 `permitAll`로 두고 있고
(주석에 "운영 배포 시 노출 범위는 별도 논의 필요"라고 적혀 있다), `springdoc` 비활성화 설정도 없다.

오픈소스 프로젝트라 API 문서 공개 자체는 오히려 바람직할 수 있다. 문제는 **셀프호스터가 고를 수
없다는 것**이다. 사내용 비공개 인스턴스를 띄운 사람에게는 전체 엔드포인트 목록이 그대로 노출된다.

**수정 방향**: `SWAGGER_ENABLED` 환경변수로 빼고 기본값을 정한다. 하드코딩 전수 조사(#229)에 묶으면 된다.
...
새 위험**(§3·§4·§5)이다. 앞의 둘은 지금 고치는 편이 싸다 — 토큰 형식이 바뀌면 FE도 함께 움직여야 하는데,
